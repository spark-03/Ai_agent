...
def render_task_engine(client):
    ...
    if task_type == "EMI Calculator":
        ...
        if st.button("Check Weather"):
            try:
                w_url = f"https://api.openweathermap.org/data/2.5/weather?q={city}&appid={st.secrets['OPENWEATHER_API_KEY']}&units=metric"
...
def render_location_weather():
    ...
    if st.button("Fetch Current Location"):
        ...
        if lat is not None and lon is not None:
            ...
            try:
                w_url = f"https://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&appid={st.secrets['OPENWEATHER_API_KEY']}&units=metric"
                w_res = requests.get(w_url, timeout=5).json()
