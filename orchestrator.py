...
class Orchestrator:
    ...
    def fetch_angel_one_data(self, query):
        return {
            "symbol": "RELIANCE",
            "price": 2950.00,
            "sma": 2910.50,
    ...
    def send_whatsapp_message(self, message_body):
        if not self.account_sid or not self.auth_token or not self.my_phone_number:
            raise KeyError("Twilio configuration is missing.")
            ...
            if is_stock_query:
                with st.chat_message("assistant"):
                    with st.spinner("Accessing Angel One SmartAPI..."):
                        data = self.fetch_angel_one_data(user_input)
                        if "message" in data:
                    ...
                    if any(word in task_lower for word in ["whatsapp", "send", "ans", "reminder"]):
                        with st.spinner("Sending answer to WhatsApp..."):
                            try:
                                ...
                                self.send_whatsapp_message(message_body)
        ...
        if st.session_state.uploaded_file_data is not None:
            if st.session_state.uploaded_file_name.endswith(".txt"):
                ...
                if st.button("Analyze Text"):
                    response = self.generate_content(f"Given this content:\n{file_content}\n\nTask: {prompt}")
                ...
                if st.button("Analyze Data"):
                    ...
                    response = self.generate_content(f"Dataset sample:\n{data_slice}\n\nTask: {prompt}")
                    st.write("### Data Analysis Result:", response.text)

    def render_math_solver(self):
        ...
        if st.button("Solve Problem"):
            if problem_input:
                ...
                response = self.generate_content(f"{instruction}\n\nProblem:\n{problem_input}")
                st.write("### Solution:", response.text)

    def render_math_quiz(self):
    ...
    def render_financial_analyzer(self):
                ...
                with st.spinner("Fetching live market metrics..."):
                    try:
                        ...
                        if "Global Quote" in data and data["Global Quote"]:
                            ...
                            res = self.generate_content(f"Provide a trend analysis summary for the financial asset {symbol}")
                            st.write(res.text)
                    except Exception as e:
                        st.error(f"Error fetching data: {e}")

    def render_task_engine(self):
        ...
        if task_type == "EMI Calculator":
            ...
            if st.button("Convert Live Currency"):
                try:
                    converted_val = self.convert_currency(amount, from_curr, to_curr)
    ...
    def render_whatsapp_agent(self):
                ...
                with st.spinner("Processing task..."):
                    ...
                    if "add" in task_lower or "+" in task_lower:
                        ...
                        if numbers:
                            ...
                            try:
                                response = self.generate_content(task_input)
    ...
    def render_location_weather(self):
        ...
        if st.button("Fetch Current Location"):
            ...
            if lat is not None and lon is not None:
                ...
                try:
                    w_res = self.get_weather_by_coords(lat, lon)
